...
if __name__ == '__main__':
    ...
    parser.add_argument("--predict_dir", default='/gcn_rw', type=str)
    ...
    num_class = 21
    if not args.curve:
        loglist = do_python_eval(args.predict_dir, args.gt_dir, name_list, num_class, args.type, args.t, printlog=True)
        writelog(args.logfile, loglist, args.comment)
    else:
        l = []
        for i in range(100):
            t = i / 100.0
            loglist = do_python_eval(args.predict_dir, args.gt_dir, name_list, num_class, args.type, t)
