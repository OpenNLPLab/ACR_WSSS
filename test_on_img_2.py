...
from myTool import compute_joint_loss, compute_seg_label, compute_cam_up, decode_segmap, compute_seg_label_old
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('vit_resnet_cls_hide_2.pth'), strict = False)
    model.cuda()
    ...
    for iter in range(len(img_list)):
        ...
        x, x_res, res_cam, cam_list = model.forward_cam_multiscale(img)
        # x, x_res, res_cam = model.forward_cam(img)
        print(res_cam.shape)
        ...
        for target_class in keys:
            ...
            cv2.imwrite(os.path.join('/home/users/u5876230/ete_project/ete_output/resnet_cam/', name_list[0] + '_{}.jpg'.format(classes[target_class])), cam_output)

        for index in range(len(cam_list)):
            res_cam = cam_list[index]
            res_cam = F.upsample(res_cam, (h,w), mode='bilinear', align_corners=False)[0]
            res_cam = res_cam.detach().cpu().numpy() * label.clone().view(20, 1, 1).numpy()
            res_cam = res_cam / (np.max(res_cam, (1, 2), keepdims=True) + 1e-5)
            ...
            for target_class in keys:
                ...
                img = cv2.resize(img, (heatmap.shape[1], heatmap.shape[0] ))
                cam_output = heatmap * 0.5 + img*0.5
                cv2.imwrite(os.path.join('/home/users/u5876230/ete_project/ete_output/resnet_cam/', name_list[0] + '_{}_{}.jpg'.format(classes[target_class], index)), cam_output)


        output = x.detach().cpu()
        # print(label[0].shape, output.shape)
        accuracy = pred_acc(label[0], output)
        if accuracy < 1:
            accuracy = 0
        accuracy_list.append(accuracy)

        res_output = x_res.detach().cpu()
        accuracy_res = pred_acc(label[0], res_output)
        if accuracy_res < 1:
            accuracy_res = 0
        accuracy_list_res.append(accuracy_res)

        print(iter, '-----', accuracy,'-----', accuracy_res)
        ...
        for batch in range(b):
            ...
            res_cam_b = res_cam[batch, :]

            for class_index in range(20):
                if cur_label[class_index] > 1e-5:
                    res_cam_c = res_cam_b[class_index,:]
                    ...
                    cam, attn_list = model.generate_cam(batch, start_layer=0)
            ...
            # seg_label[batch] = compute_seg_label_old(original_img, cur_label.cpu().numpy(), norm_cam, croppings[:,:, batch], name, iter,
            #                                      save_heatmap=True)
            ...
            for target_class in keys:
                ...
                cv2.imwrite(os.path.join('/home/users/u5876230/ete_project/ete_output/resnet_cam/', name + '_{}_vit.jpg'.
                format(classes[target_class] )), cam_output)
    #    round(output_prob.detach().cpu().numpy()[target_class], 10)
    ...
    print('average resnet accuracy:', sum(accuracy_list_res)/ len(accuracy_list_res))

    print('average vit accuracy:', sum(accuracy_list)/ len(accuracy_list))
