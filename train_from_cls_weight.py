...
def train(gpu, args):
    ...
    # model.load_state_dict(torch.load('resnet_vit_cls_2.pth'), strict = False)
    ...
    for iter in range(max_step+1):
        ...
        # print(closs.item(), celoss.item(), dloss.item())
        ...
        if (optimizer.global_step-1)%50 == 0 and gpu ==0:
            ...
            vis.line(train_loss_list,
                     win='train_loss_per_iteration_train_from_cls_weight_{}'.format(args.session_name),
                     opts=dict(title='train_loss_per_iteration_train_from_cls_weight_{}'.format(args.session_name)))
