...
def train(gpu, args):
    ...
    weights_dict = torch.load('weight/train_from_init_hybrid_0.9_best.pth')
    ...
    for iter in range(max_step-1):
        ...
        if (optimizer.global_step-1) > args.cls_step * max_step:
            ...
            for batch in range(b):
                ...
                norm_cam,  name, iter, saliency_map.data.numpy(),x[batch, :], save_heatmap=True)
            
            pseudo_run_time = (time.time() - start)
            print("--- %s seconds ---" % pseudo_run_time)
            
            generate_pseudo_time_list.append(pseudo_run_time)
    ...
    print(sum(generate_pseudo_time_list))
